...
На язык LLVM IR, в отличие от MLIR @lattner2020mlir, отсутствует формальная спецификация @llvmLang. В то же время, синтаксис LLVM IR меняется с выходом новых версий фреймворка LLVM. По этой причине воссоздание формальной
...
Первым этапом осуществляется преобразование линейного участка на языке LLVM IR в ГЗ @sovetov2021development, в котором
...
Следующим этапом реализуется построение ЯПФ @fet1995parallel. Каждый ярус ЯПФ представляет собой множество всех
...
сведена к задаче программирования в ограничениях, аналогично тому, как это сделано в @sovietov2023trubol. При этом используются полученные в результате выполнения алгоритма построения ЯПФ значения d и w_max.
...
В рассматриваемом инструменте анализа степени распараллеливания решение этой задачи реализовано с помощью решателя CP-SAT из состава программного пакета OR-Tools @ORTools.
...
Время, затраченное на анализ каждой программы приведено в @tbl:tabl1 .
...
\centering
    \caption{\raggedleft{Общее время анализа программ} 
    }
    \label{tbl:tabl1}
...
\centering
    \caption{\raggedleft{Выбранные линейные участки программ} 
    }
    \label{tbl:tabl2}
...
Значения ширины ЯПФ до и после минимизации, а также коэффициенты ускорения для выбранных линейных участков из @tbl:tabl2 приведены в диаграммах на рис. 5.
...
Библиотека pyMLIR @ben2018neural реализует разбор промежуточного представления MLIR и предоставляет ряд инструментов для работы с данным представлением. Она может быть применена для некоторых задач статического
...
В @pyMLIR описан программный инструмент, реализующий разбор представления LLVM IR с использованием
